...
@dataclass
class RESTScriptArguments(ScriptArguments):
    """
    Script arguments for ReST_EM training.
...
def _combine_reward_funcs(reward_funcs):
    ...
    def combined_reward_fn(prompts, completions, **kwargs):
        ...
        for reward_func in reward_funcs:
            try:
                reward = reward_func(prompts=prompts, completions=completions, **kwargs)
            except TypeError:
...
def make_parser(subparsers: argparse._SubParsersAction | None = None):
    dataclass_types = (RESTScriptArguments, RESTConfig, ModelConfig, DatasetMixtureConfig)
    if subparsers is not None:
        parser = subparsers.add_parser("rest", help="Run the ReST_EM training script", dataclass_types=dataclass_types)
    else:
